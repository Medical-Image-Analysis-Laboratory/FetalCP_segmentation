...
from topofetal.workflows import assessment_stage


class InferenceAndEvaluationPipeline:
    """Class used to represent the workflow of inference + performance
    assessment of fetal automatic CP segmentation.
    ...
    def create_workflow(self):
        ...
        dg = pe.Node(interface=DataGrabber(outfields=dg_fields), name='data_grabber')
        dg.inputs.template = '*'
        ...
        segmentation_assessment = assessment_stage.create_assessment_stage(
            p_name='assessment_stage')
        segmentation_assessment.inputs.inputnode.input_fold = -1 # -1: testing
        ...
        if self.m_dataset in ['feta_2.0_ref']:
            self.m_wf.connect(dg, 'tiv_like',
        ...
        self.m_wf.connect(segmentation_ensemble, 'outputnode.output_prediction',
                          segmentation_assessment, 'inputnode.input_prediction')
        self.m_wf.connect(segmentation_preproc_gt, 'outputnode.output_map',
                          segmentation_assessment, 'inputnode.input_ground_truth')

        self.m_wf.connect(segmentation_assessment, 'outputnode.output_metrics',
                          datasinker, 'anat.@output_metrics')
        self.m_wf.connect(segmentation_assessment, 'outputnode.output_cortex_with_holes',
                          datasinker, 'anat.@output_cortex_with_holes')
